...
class VariationalDropoutLSTMCell(nn.Module):
    ...
    def resample_mask(self, batch_size):
        if self.recurrent_drop_out_rate > 0.0 and self.training:
            self.dropout_mask_if = torch.bernoulli(
                1 - self.recurrent_drop_out_rate * torch.ones(batch_size, self.input_size + self.hidden_size))
    ...
    def forward(self, input, state):
        ...
        if self.recurrent_drop_out_rate > 0.0 and self.training:
            f_in = combined_inputs * self.dropout_mask_if / (1 - self.recurrent_drop_out_rate)
            i_in = combined_inputs * self.dropout_mask_ii / (1 - self.recurrent_drop_out_rate)
            o_in = combined_inputs * self.dropout_mask_io / (1 - self.recurrent_drop_out_rate)
            g_in = combined_inputs * self.dropout_mask_ig / (1 - self.recurrent_drop_out_rate)
        else:
