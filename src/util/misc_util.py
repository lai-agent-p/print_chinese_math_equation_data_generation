...
def normal_sample(left_boundary, right_boundary):
    #99.73 percent in interval
    width = right_boundary - left_boundary
    realization = np.random.normal((left_boundary + right_boundary)/2, (width/3), size=1)[0]
    if realization < left_boundary:
        realization = left_boundary
    if realization > right_boundary:
        realization = right_boundary
    return realization
...
def get_dict_entry(token, config):
    if not token in config.label2unicode:
        return token
    else:
        return random.choice(config.label2unicode[token])
